...
def create_dictionary_mnn(adata, use_rep, use_label, batch_name, k = 50,  verbose = 1, mask_rate=0.5):
    ...
    for idx, b_name in enumerate(batch_list.unique()):
        ...
        names1 = new
        names2 = ref
        match, u_unique_set, n_unique_set = MNN(ds1, ds2, gt1, gt2, names1, names2, u_unique_set, n_unique_set, knn=k, mask_rate=mask_rate)
        if (verbose > 0):
...
def MNN(target_slice_ds, rest_slice_ds, gt1, gt2, names1, names2, u_unique_set=None, n_unique_set=None, knn = 20, approx = False, mask_rate=0.5):
